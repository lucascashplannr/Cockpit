...
/** §3.4 — probe, never remember: the declared state is re-read, not cached. */
function manifestOf(project: Project): ManifestV1 | null {
  return project.manifestPath ? readManifest(project.manifestPath).manifest : null
}
...
/** Read on request for the details sheet; see `WorkspaceDetails`. */
export async function workspaceDetails(id: string): Promise<WorkspaceDetails | null> {
  const w = workspaces.get(id)
  if (!w) return null
  let createdAt: number | null = null
  try {
    const st = statSync(w.path)
    // Zero is what a filesystem without birth times reports.
    createdAt = st.birthtimeMs > 0 ? st.birthtimeMs : null
  } catch {
    createdAt = null
  }
  if (!isRepo(w.path)) return { createdAt, remoteUrl: null, head: null }

  const [remote, log] = await Promise.all([
    git(w.path, ['remote', 'get-url', 'origin']),
    git(w.path, ['log', '-1', '--format=%H%x00%s%x00%an%x00%ct']),
  ])
  const [sha, subject, author, ct] = log.ok ? log.stdout.trim().split('\0') : []
  return {
    createdAt,
  ...
  for (const main of mains) {
    ...
    for (const entry of entries) {
      ...
      const ws: Workspace = {
        ...
        path: entry.path,
        // The one place the link to the main checkout exists: worktrees are
        // enumerated *from* it, and the path alone cannot say which repo this
        // is a checkout of (§8).
        repoName: basename(main.path),
        repo: entry.path,
        git: null,
        runtime: null,
        topicId: null,
        capabilities: detectCapabilities(entry.path, manifestOf(project), basename(main.path)),
        agentSessions: [],
        lease: null,
...
function addTopicRoots(project: Project): void {
  ...
  for (const rec of topicStore.list(project.id)) {
    ...
    const ws: Workspace = {
      ...
      name: rec.name,
      path: rec.rootPath,
      /** A topic folder is not a checkout of a repository (§8). */
      repoName: '',
      repo: null,
      git: null,
      runtime: null,
...
/**
 * The structural half of a pass: which workspaces and topics exist, and where.
 * No probe, no network — one `git worktree list` per repository and a handful
 * of `existsSync`, so it finishes in milliseconds where a full pass takes
 * seconds because it goes to origin first.
 *
 * It exists so the service can be structurally complete *before* it starts
 * listening (§13 rule 3 is "probe at boot", not "serve a registry that is
 * still empty"). A window that connected inside the probe window was handed an
 * empty list and told "unknown workspace" about workspaces that were on disk
 * the whole time; the ids answer from the first `hello` now, and the probe
 * fills in `git` and `runtime` behind them.
 */
export async function hydrate(projectId?: string): Promise<void> {
  for (const root of rootsFor(projectId)) {
...
function rootsFor(projectId?: string): string[] {
  if (projectId) return [projects.get(projectId)?.root].filter((x): x is string => !!x)
  return loadConfig().projects.map((p) => p.root)
}
